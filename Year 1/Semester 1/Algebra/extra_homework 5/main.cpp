...
// Because we are working with vector spaces over Z_2, we can view each vector as a binary representation of a number,
// and we can use the XOR operator as the equivalent of the sum between 2 vectors.
// All the numbers range from 1 to (2 ^ n - 1).
// For a vector to be part of the basis, the condition is that the XOR result with any previous vectors
// from the basis has to be greater than the vector that is already in the possible basis.
// If this condition is not met, the vector ca not be part of the basis.
bool check_solutions(int y)
{
  for (auto x : solution)
  {
    if ((x ^ y) < y)
