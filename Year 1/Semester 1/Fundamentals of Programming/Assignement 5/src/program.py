...
def longest_subarray(l: list, arr: list):
    ...
    :return: the length of the longest subarray that contains at most 3 distinct values(int)
    """
    length = 0
    i_start = -1
    ok = False
...
def longest_increasing_subseq(m: list, s: list, seq: list):
    ...
    :return: the length of the longest increasing subsequence(int)
    """
    aux = []
    for i in m:
        aux.append(-1)
...
def read_the_numbers(l: list):
    ...
    print("Your numbers have been added to the list")

def display_the_list(l: list):
    for i in range(len(l)):
        if i != len(l) - 1:
            print(to_string(l[i]), end=", ")
        else:
            print(to_string(l[i]))
