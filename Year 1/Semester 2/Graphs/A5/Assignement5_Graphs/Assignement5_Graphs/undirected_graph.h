...
public:
	...
	/// <summary>
	/// This returns an hamiltonian cycle, if there is one, starting
	/// from a specific vertex
	/// </summary>
	/// <returns>the cycle, if there is one</returns>
	std::vector<int> FindHamiltonianCycle()
	{
		// a vector to know if a specific vertex was already
		// added to the cycle
		std::vector<bool> visited(get_nr_of_vertices());

		// initially, all are false
		std::fill(visited.begin(), visited.end(), false);  
		std::vector<int> cycle;   // the cycle               

		// find the first vertex from the list of vertices
		int vert = -1;
		...
		// add the vertex to the cycle
		cycle.push_back(vert);
		visited[vert] = true; // set it as visited

		// The algorithm which tells us if we found a Hamiltonian cycle
		bool res = HamiltonianCycle(visited, cycle, vert); 

		// if we found one we return it
		if (res)
			return cycle;

		// if we didn't find one return an empty vector
		return std::vector<int>();
	}

	/// <summary>
	/// This function tells us if we have a Hamiltonian cycle.
	/// Also, it uses backtracking going through all the adjacent
	/// vertices by recursion.
	/// </summary>
	/// <param name="visited"></param>
	/// <param name="cycle"></param>
	/// <param name="node"></param>
	/// <returns></returns>
	bool HamiltonianCycle(std::vector<bool>& visited, std::vector<int>& cycle, 
		int node)
	{
		// the initial condition to see if have found
		// a Hamiltonian cycle
		// if we found one we stop
		if (cycle.size() == get_nr_of_vertices() &&
			is_edge(node, cycle[0]))
			return true;

		// we go through all the adjacent vertices
		for (auto ne : get_adjacent_vertices(node))
			// if the vertex is not already in the cycle
			if (!visited[ne])
			{
				visited[ne] = true;  // set it as visited
				cycle.push_back(ne); // put it in the cycle

				// go to the next vertex
				if (HamiltonianCycle(visited, cycle, ne)) 
					return true;

				//if we didn't found a Hamiltonian cycle
				// remove the ertex from the cycle
				visited[ne] = false; 
				cycle.pop_back();
			}

		// if we didn't find any Hamiltonian cycle
		return false;
	}
